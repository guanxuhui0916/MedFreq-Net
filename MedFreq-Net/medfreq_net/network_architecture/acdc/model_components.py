...
from medfreq_net.network_architecture.acdc.transformerblock import TransformerBlock
...
class MedFreqNetEncoder(nn.Module):
    def __init__(self, input_size=[16 * 40 * 40, 8 * 20 * 20, 4 * 10 * 10, 2 * 5 * 5],dims=[32, 64, 128, 256],spatial_shape =[(16,40,40), (8,20,20), (4,10,10),(2,5,5)],
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                stage_blocks.append(TransformerBlock(input_size=input_size[i], hidden_size=dims[i],
                                                     proj_size=proj_size[i], spatial_shape=spatial_shape[i], num_heads=num_heads,
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(TransformerBlock(input_size=out_size, hidden_size=out_channels,
                                                     proj_size=proj_size, spatial_shape=spatial_shape, num_heads=num_heads,
