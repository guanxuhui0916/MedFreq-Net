...
from medfreq_net.network_architecture.layers import LayerNorm
from medfreq_net.network_architecture.synapse.transformerblock import TransformerBlock
from medfreq_net.network_architecture.dynunet_block import get_conv_layer, UnetResBlock


einops, _ = optional_import("einops")

class MedFreqNetEncoder(nn.Module):
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4],dims=[32, 64, 128, 256], spatial_shape =[(32,32,32),(16,16,16),(8,8,8),(4,4,4)],
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                stage_blocks.append(TransformerBlock(input_size=input_size[i], hidden_size=dims[i],  proj_size=proj_size[i],spatial_shape=spatial_shape[i], num_heads=num_heads,
    ...
    def     __init__(
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(TransformerBlock(input_size=out_size, hidden_size= out_channels, proj_size=proj_size, spatial_shape=spatial_shape, num_heads=num_heads,
